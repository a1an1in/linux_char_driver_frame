...
int pfs_set_proto_format_3052(protocol_format_set_t *pfs_p)
{
	...
	PFS_SET_PROTO_INFO( SLC_INFO, 	  "9", "0", "1", "8",  NULL,     hl_head);
	PFS_SET_PROTO_INFO( SLC_DATA,     "10","0", "0", "8",  SLC_INFO, hl_head);
	PFS_SET_PROTO_INFO( EMB_INFO,     "10","0", "1", "8",  NULL,     hl_head);
	PFS_SET_PROTO_INFO( LC_INFO,      "11","0", "1", "8",  NULL,     hl_head);
	PFS_SET_PROTO_INFO( LC_DATA,      "12","0", "0", "8",  LC_INFO,     hl_head);
	PFS_SET_PROTO_INFO( DC_INFO,      "12","0", "1", "8",  NULL,     hl_head);
	PFS_SET_PROTO_INFO( DC_DATA,      "13","0", "0", "8",  DC_INFO,  hl_head);

	llist_num =0x3052 - pfs_p->proto_base_addr;
	pfs_add_proto_link_list(llist_num,
			hl_head,//struct list_head *new_proto_llist,
			pfs_p);//struct list_head **list_head)
...
void pfs_set_pdt_drv_proto_format(protocol_format_set_t *pfs_p)
{
	...
	pfs_set_proto_format_3050(pfs_p);
	pfs_set_proto_format_3052(pfs_p);
	pfs_set_proto_format_3054(pfs_p);
}
