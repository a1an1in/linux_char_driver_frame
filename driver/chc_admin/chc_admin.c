...
chc_admin_t *create_chc_admin()
{
	...
	admin = allocator_mem_alloc(allocator,sizeof(chc_admin_t));
	if(admin == NULL){
 ...
 *};
 */
static inline int8_t get_chc_state(chc_admin_t *admin,uint8_t type)
{
	return 1;
}
int get_task(chc_admin_t *admin,uint8_t flag,uint16_t slot_num,task_t **task)
{
	...
	if(flag == TASK_FLAG_RCV){
		...
		if(get_chc_state(admin,CHC_STATE_MAINC_CHANNEL)){
			...
			//... periodic task
		}else if(get_chc_state(admin,CHC_STATE_VOICE_CHANNEL)){
		}else if(get_chc_state(admin,CHC_STATE_DATA_CHANNEL)){
		}else{
			dbg_str(DBG_ERROR,"not support");
		}
	}else{
